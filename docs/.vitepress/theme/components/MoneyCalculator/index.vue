<template>
  <div class="vp-raw">
            ...
            <el-table class="w-full!" :data="form.inouts" border>
              ...
              <el-table-column label="收支">
                <template #default="{ row, $index }">
                  <el-row :gutter="8">
                    ...
                    <el-col :span="span6">
                      ...
                      >
                        ...
                        >
                          ...
                          <el-option
                            label="每月"
                            :value="MoneyIOPer.PER_MONTH"
                          />
                        </el-select>
                      </el-form-item>
                    </el-col>
                    <el-col
                      v-if="row.per === MoneyIOPer.PER_MONTH"
                      :span="span6"
                    >
                      <el-form-item
                    ...
                    <el-col
                      v-if="row.per === MoneyIOPer.PER_DATE"
                      :span="span6"
                    >
                      <el-form-item
              ...
              <el-table-column label="操作" width="90">
                <template #default="{ $index }">
                  <el-button
                    type="danger"
                    @click="form.inouts.splice($index, 1)"
                  >
                    删除
                  </el-button>
                </template>
    ...
    <el-card class="mt-32px" header="收支记录" shadow="nevel">
      <el-table :data="records">
        <el-table-column label="日期" prop="date" />
        <el-table-column label="收支">
          <template #default="{ row }">
            <div
              v-if="row.type === MoneyIOType.IN"
              style="color: var(--el-color-success)"
            >
              +{{ row.value }}
            </div>
            <div
              v-else-if="row.type === MoneyIOType.OUT"
        ...
        <el-table-column label="余额" prop="total" />
      </el-table>
    </el-card>
  </div>
</template>
...
import { calcRecords, createForm } from "./utils";

const { spanOne, spanFull, span6 } = useSpan();

const formRef = ref<FormInstance>();
const form = ref(createForm());
const records = shallowRef<MoneyIORecord[]>([]);
...
const onClickCalc = async () => {
  ...
  records.value = calcRecords(form.value);
};
</script>
